...
public class MainController
{
	public static void main(String[] args) throws IOException, InterruptedException
	{
		...
		{	
	        ...
	        savePath = roots[1] + "Prntsc";
		    File directory = new File(savePath);
		    if (!directory.exists())
		        directory.mkdir();
		    
			while(System.in.available() == 0) //Close when user press enter
			{
				url = GetURL();
				doc = Jsoup.connect(url).get();
				Elements div = doc.select("div.image-container");
				Elements link = div.select("img"); 
				
				String src = link.attr("src");
				//System.out.println("UNDER DIV: " + div);
				//System.out.println("IMG DIV: " + link);
				System.out.println("SOURCE: " + src);

					if(!src.contains(missing) || src != missing)//Check if missing image is displayed
						DownloadImage(src, index, savePath);
		...
		{
			System.out.println("ERROR: Could not get path to image.");
		}
	}
	
	public static void DownloadImage(String link, int index, String savePath) 
	{
		URL url;
		try
		{
			...
			OutputStream out = new BufferedOutputStream(new FileOutputStream(savePath + "\\Image" + Integer.toString(index) + ".jpg"));
			...
			in.close();
			out.close();
			System.out.println("SUCCESS: Saved image in: " + savePath);
		} 
		catch (IOException e)
		{
			e.printStackTrace();
	...
	{
		StringBuilder sb = new StringBuilder("https://prnt.sc/");
		String sub = GenerateSub();
		sb.append(sub);
		
		return sb.toString();
	}
	
	public static String GenerateSub() 
	{
		String[] arr = {"1", "2", "3", "4", "5", "6", "7", "8", "9", "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z" };
		StringBuilder sb = new StringBuilder("");
		Random r = new Random();
		int num;
		for(int i = 0; i < 6; ++i) 
		{
	      	num = r.nextInt(arr.length);
	      	sb.append(arr[num]);
		}
		return sb.toString();
	}
